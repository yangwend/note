...
const FirstTable: FC<IProps> = ({
  ...
  const syncTbodyHeight = () => {
    ...
    tbodyLeftItems.forEach((childItem, childIndex) => {
      // 注意！由于第6/7项可能存在子组件渲染，直接获取外层div高度无法拿到真实子组件的高度，故两者取最大来处理
      // TODO:自定义组件，高度设置有问题
      if (childIndex % 27 === 5) {
        const childHeight =
          (childItem.querySelector(`.${styles.purchaseInput}`) as HTMLDivElement)?.offsetHeight ??
          0;
      ...
      } else if (childIndex % 27 === 6) {
        const childHeight =
          (childItem.querySelector(`.${styles.goodsStatusInput}`) as HTMLDivElement)
            ?.offsetHeight ?? 0;
        tbodyLeftItemHeights.push(
          Math.max(
            (childItem as HTMLDivElement).offsetHeight,
  ...
  }, [activeKey, firstList, bottomHeight]);
          ...
          <div className={styles.tbodyLeft} ref={tbodyLeftRef}>
            <div className={styles.tbodyLeftWrap} ref={tbodyLeftItemRef}>
              <div className={styles.tbodyLeftItems}>
                ...
                <div className={styles.tbodyLeftItem}>当前让他分类</div>
                <div className={styles.tbodyLeftItem}>当前让他状态</div>
                <div className={styles.tbodyLeftItem}>申请调整让他分类</div>
                <div className={styles.tbodyLeftItem}>申请调整让他状态</div>
                <div className={styles.tbodyLeftItem}>让他分类审批意见</div>
                <div className={styles.tbodyLeftItem}>让他状态审批意见</div>
              ...
              <div className={`${styles.tbodyLeftItems} ${styles.tbodyLeftItems1}`}>
                <div className={styles.tbodyLeftItem}>{firstList[0].我去32332 || '/'}</div>
                ...
                <div className={styles.tbodyLeftItem}>{firstList[0].auditPurchaseName || '-'}</div>
                <div className={styles.tbodyLeftItem}>
                  ...
                  <div className={styles.tbodyRightItem}>
                    {item.dataType === '1' ? (
                      ...
                      <GoodsStatusPicker
                        ...
                        className={styles.goodsStatusInput}
                        onDescChange={() => {
                          if (activeKey === '1') {
                            syncTbodyHeight();
                          }
