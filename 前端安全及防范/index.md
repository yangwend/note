...
```javascript
// 比如说用户访问一个页面，页面链接携带参数name，在页面代码中，需要获取name参数并展示在页面上。如果此时name为<script>alert(1)</script>，而代码中没有在对name做任何处理的话，就会执行alert(1)这段代码
...
当content为`<script>alert(1)</script>`时，此时页面这段代码就变为`<div><script>alert(1)</script></div>`，此时页面就有可能弹窗1。

防御策略：将 < 和 > 转义成 `&lt;` 和 `&gt;`。

转义之后，页面这段代码就变为 `<div>&lt;script&gt;alert(1)&lt;/script&gt;</div>`，就可以避免了。
...
当imgSrc的内容为`2" onerror="alert(2)`，此时页面这段代码就变为`<img src="2" onerror="alert(2)" />`。由于src为2，img加载失败，会执行onerror方法，此时就会弹窗2了。

防御策略：将 " 转义成 `&quto;`。

转义之后，页面这段代码就变为`<img src="2&quto; onerror=&quto;alert(2)" />`，就可以避免了。
...
当 data 内容为`hello";alert(3);"`，脚本攻击后，会变成：
```javascript
<script>
    const mydata = "mhello";alert(3);"";
</script>
```
页面将会执行alert(3)。
...
上述都可以进行标签转义实现todo：[【JS】JS实现Html转义和反转义（html编码和解码）的方法总结](https://www.cnblogs.com/willingtolove/p/11059325.html)

4. 富文本<br/>
富文本需要保留 HTML 文本，HTML 文本中就有 XSS 攻击的风险。

防御策略：使用白名单保留部分标签和属性，其余标签和属性进行过滤和转义处理。
...
其原理就是攻击者构造出一个后端请求地址，诱导用户点击或者通过某些途径自动发起请求。如果用户是在登录状态下的话，后端就以为是用户在操作，从而进行相应的逻辑。

```html
<img src="http://www.domain.com/xxx?comment='attack'"/>
```
假如某个网站有一个使用GET方式请求的接口，用于修改用户的评论。那攻击者可以通过在自己的网站或者钓鱼网站中加入一个图片，如上所示。此时用户只要点击该图片，评论就会被任意修改。
...
5. 不让第三方网站访问用户的cookie信息：对cookie设置`SameSite`属性。
...
遇到这种情况，网站开发人员也没办法处理，只能说，可以借鉴一下天猫这种方案，去提示下用户了。
...
todo [前端黑魔法之远程控制地址栏](https://www.leavesongs.com/PENETRATION/use-target-to-spoof-fishing.html)
...
2. 针对CSRF攻击：不使用GET方式修改数据、对cookie进行签名和加密、对cookie设置SameSite防止跨域请求发送、接口请求时附带验证码或者token并进行token校验等。
...
5. [Web 漏洞分析与防御之点击劫持（三）](https://juejin.im/entry/59e40c05f265da43127ff4c7)

6. [前端的安全问题与防御策略](https://juejin.im/post/5cd373bee51d456e671c7e70)
...
？. [Web安全系列（四）：XSS 的防御](https://juejin.im/post/5bac9e21f265da0afe62ec1b)