...
export default {
  ...
  methods: {
    ...
    // tab切换值
    tabClick(tab) {
      this.tabActive = tab && tab.props && tabs.props.name || this.tabActive;
    },
    ...
    updateContabValue(name, value, from) {
      ...
      if (from && from !== name) {
        console.log(`来自组件 ${from} 改变了 ${name} 的值： ${value}`);
    ...
    changeRadio(name, value) {
      ...
      let insValue; // 组件内部单选选项选中值

      if (!this.$refs[refName]) return;

      if (arr.includes(name)) {
        // second、min、hour、mouth 通配符：, - * /
        if (value === "*") {
          insValue = 1;
  ...
  watch: {
    expression: "resolveExp", // 反解析 表达式
    hideComponent(value) {
      // 隐藏部分组件
    },
  },
  mounted: function() {
    this.resolveExp(); // 反解析 表达式
  },
};
</script>
<style scoped>
